define(["require", "exports", "../wafle", "../wafleDataBlob"], function(require, exports, Wafle, wafledata) {
    exports.Types = wafledata.WAFLE_DATA_BLOB_INVENTORY_TYPES_BY_GROUP;

    exports.TypeToGroupIDMapping = wafledata.WAFLE_DATA_BLOB_MARKET_GROUPS;

    function RootMarketGroups() {
        return exports.ChildMarketGroupsByID(undefined);
    }
    exports.RootMarketGroups = RootMarketGroups;
    function WafleRootMarketGroups() {
        var mgs = [exports.MarketGroupByID(4), exports.MarketGroupByID(9), exports.MarketGroupByID(11), exports.MarketGroupByID(955), exports.MarketGroupByID(157)];
        for (var i = 0; i < mgs.length; i++) {
            mgs[i] = DeriveAllChildMarketGroups(mgs[i]);
        }
        return mgs;
    }
    exports.WafleRootMarketGroups = WafleRootMarketGroups;

    function DeriveAllChildMarketGroups(theMarketGroup) {
        var mg = theMarketGroup;
        if (!mg.children) {
            mg.children = [];
        }
        var cmg = exports.ChildMarketGroupsByID(theMarketGroup.mgid);
    ...
    function MarketGroupByID(marketGroupID) {
        for (var i = 0; i < exports.TypeToGroupIDMapping.length - 1; i++) {
            if (exports.TypeToGroupIDMapping[i].mgid === marketGroupID) {
                return exports.TypeToGroupIDMapping[i];
            }
        }
        return null;
    }
    exports.MarketGroupByID = MarketGroupByID;

    function ChildMarketGroupsByID(parentMarketGroupID) {
        var marketGroups = [];
        for (var i = 0; i < exports.TypeToGroupIDMapping.length - 1; i++) {
            if (exports.TypeToGroupIDMapping[i].pgid === parentMarketGroupID) {
                marketGroups.push(exports.TypeToGroupIDMapping[i]);
            }
        }
        return marketGroups;
    }
    exports.ChildMarketGroupsByID = ChildMarketGroupsByID;
